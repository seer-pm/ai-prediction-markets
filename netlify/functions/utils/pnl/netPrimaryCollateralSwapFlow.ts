...
export type PrimaryCollateralSwapFlowByPeriod = {
   ...
   *
   * Fork divergence (upstream counts the primary leg only): on conditional markets — Round 2 · L2
   * and Round 2 · Originality — the pool is `childOutcome ↔ parentOutcome`, so no leg is ever the
   * primary collateral and upstream's rule drops every swap, leaving those leaderboards at
   * `volume: 0`, `marketCount: 0` and an undefined ROI.
   *
   * Parent outcome tokens have no pool against the primary collateral, so there is no traded price
   * for them at any point in the window — but both Round 2 parents have resolved, and settlement
   * *is* an exact primary-collateral value (a winning L2 repo token redeems for 1/15, Originality
   * 1/33). Counting them at face value instead would overstate by 15× / 33×, and by ~99× against
   * what a split paid per token, which would rank these contests far above Round 1 on the summed
   * global board. The cost of settlement pricing is that legs against an outcome that lost, or
   * against a parent that has not resolved, price at 0 and drop out.
   */
  volumeByStartTime: Map<number, number>;
  /**
   * Primary collateral spent buying outcomes (primary as `tokenIn`) per window start.
   *
   * Stored rather than derived: the leaderboard's ROI capital used to recover buys from
   * `(volume + netOut) / 2`, an identity that only holds while volume counts primary legs alone.
   */
  buysByStartTime: Map<number, number>;
  /** Distinct markets with counted swap volume per window start. */
  marketCountByStartTime: Map<number, number>;
  /**
   * The market ids behind `marketCountByStartTime`. Fork-only addition (upstream keeps just the
   * count): the leaderboard merges a participant's wallets, and adding two counts double-counts a
   * market both traded — it produced `marketCount: 2` inside a one-market contest.
   */
  marketIdsByStartTime: Map<number, string[]>;
...
function aggregateSwapFlowForPeriods(
      ...
      if (rowLimit > 0) {
        ...
        if (rows.length < rowLimit) {
          rows.push({
            ...
            countedVolumePrimary: collateralIsPrimary
              ? Number(formatUnits(volumeCounted, decimals))
              : volumePriced,
          });
  ...
  const marketIdsByStartTime = new Map<number, string[]>();
  for (const st of startTimes) {
    ...
    marketIdsByStartTime.set(st, [...(marketsByStart.get(st) ?? [])]);
  }
...
/**
 * Net **primary collateral** spent on outcome **DEX** swaps in `(startTime, endTime]`,

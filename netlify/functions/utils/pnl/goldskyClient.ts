...
import {
  GET_ACCOUNT_DEX_EVENTS,
  type GetAccountDexEventsQuery,
  type GetAccountDexEventsQueryVariables,
} from "./dexQueries";
...
/** The SDK builds these clients, so take the type from it rather than depending on `graphql-request` directly. */
type DexGraphQLClient = NonNullable<ReturnType<typeof swaprGraphQLClient>>;

type DexSdk = ReturnType<typeof getSwaprSdk> | ReturnType<typeof getUniswapSdk>;

const dexSdkByChain = new Map<number, DexSdk>();

export function getDexGraphQLClient(chainId: SupportedChain): DexGraphQLClient {
...
/**
 * `GetAccountDexEvents` is not in the published SDK (see `dexQueries.ts`), so it is issued as a
 * raw request against the same client and through the same pacing/retry wrapper the generated
 * operations use. Swaps, mints and burns come back in one POST.
 */
export async function getAccountDexEvents(
  chainId: SupportedChain,
  variables: GetAccountDexEventsQueryVariables,
): Promise<GetAccountDexEventsQuery> {
  const client = getDexGraphQLClient(chainId);
  return runGoldskyDexRequest(
    () => client.request<GetAccountDexEventsQuery>(GET_ACCOUNT_DEX_EVENTS, variables),
    "GetAccountDexEvents",
  );
}

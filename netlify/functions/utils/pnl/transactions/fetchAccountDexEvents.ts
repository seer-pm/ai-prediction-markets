...
import type { GetAccountDexEventsQuery } from "../dexQueries";
import { getAccountDexEvents } from "../goldskyClient";
...
async function paginateCombinedTokenChunk(args: {
  ...
  while (!swapDone || !mintDone || !burnDone) {
    ...
    const data = await getAccountDexEvents(args.chainId, variables);
 ...
 *
 * Upstream also merges CoW Protocol fills here; that leg is dropped in this fork (deep markets
 * are Optimism-only and trade on Uniswap v3) — see `../README.md`.
 */
export async function fetchAccountDexEvents(
  ...
  return fetchDexEventsFromSubgraph(mappings, account, chainId, startTime, endTime);
}

...
/**
 * Run a raw HyperIndex document through the same pacer and 429 retry as the generated SDK.
 *
 * The published `@seer-pm/sdk` is behind Seer's workspace copy: `GetAccountActivity`,
 * `GetTokenBalances`, `GetTokenBalanceDailies` and the `market` field on `Transfer` are not in
 * its generated operations. Those documents live in `envioQueries.ts` and go through here.
 */
export async function seerEnvioRequest<T>(
  ...
  return envioSdkWrapper(() => client.request<T>(document, variables), operationName, "query") as Promise<T>;
}
